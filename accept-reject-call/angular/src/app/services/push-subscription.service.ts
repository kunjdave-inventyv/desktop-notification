...
export class PushSubscriptionService {
  ...
  /**
   * Listen for messages the service worker posts back to the app.
   * Handles:
   *   - CALL_ACCEPT_FROM_NOTIFICATION  — user tapped Accept on a push notification
   *   - CALL_REJECT_FROM_NOTIFICATION  — user tapped Reject on a push notification
   */
  onSwMessage(callback: (data: any) => void): void {
  ...
  /**
   * Tell the service worker to close any open notification for `from`.
   * Called when:
   *   - Edge #3: Caller cancelled the call → dismiss callee's push notification.
   *   - Edge #5: One tab accepted/rejected → dismiss notifications on all other tabs.
   */
